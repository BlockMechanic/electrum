...
class ElectrumWindow(QMainWindow):
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum's focus is speed, with low resource usage and simplifying Bitcredit. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Bitcredit system."))

    def show_report_bug(self):
        QMessageBox.information(self, "Electrum - " + _("Reporting Bugs"),
            _("Please report any bugs as issues on github:")+" <a href=\"https://github.com/bitcreditscc/electrum/issues\">https://github.com/bitcreditscc/electrum/issues</a>")
    ...
    def create_send_tab(self):
        ...
        msg = _('Recipient of the funds.') + '\n\n'\
              + _('You may enter a Bitcredit address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Bitcredit address)')
        ...
        msg = _('Bitcredit transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def read_send_tab(self):
        ...
        for _type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('Bitcredit Address is None'), _('OK'))
                return
            if _type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid Bitcredit Address'), _('OK'))
                return
            if amount is None:
